...
var resizePizzas = function (size) {
    ...
    */
    ...
    function changePizzaSizes(size) {
        // Only get all the pizza elements one time
        var allPizzas = document.getElementsByClassName("randomPizzaContainer");

        var newWidth;
        // change pizza width to match 'size' from the slider bar
        switch (size) {
            case "1":
                newWidth = 25; //small - 4 per row
                break;
            case "2":
                newWidth = 33; //medium - 3 per row
                break;
            case "3":
                newWidth = 50; //large - 2 per row
                break;
            default:
                console.log("bug in sizeSwitcher");
                break;
        ...
        //eliminated the repeated querySelectorAll() usage in this loop
        //set the width for all of the pizzas
...
// With some help from https://discussions.udacity.com/t/project-4-how-do-i-optimize-the-background-pizzas-for-loop/36302
function updatePositions() {
    frame++;
    window.performance.mark("mark_start_frame");

    //get all the background pizzas into one array
    var allBGPizzas = document.getElementsByClassName('mover');

    //we'll use this in our phase calcs
    var top = document.body.scrollTop;

    //there are only 5 possible 'phase' values, so we put them in constArray so we don't have to repeatedly recalculate
    var phaseArray = [];
    for (var i = 0; i < 5; i++) {
        phaseArray.push(Math.sin((top / 1250) + i));
    }

    for (var i = 0; i < allBGPizzas.length; i++) {
        //now we can adjust the background pizzas' positions using our pre-calculated phase values
        allBGPizzas[i].style.left = allBGPizzas[i].basicLeft + 100 * phaseArray[i % 5] + 'px';
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Use browser window to calculate number of rows and columns
    // Help from https://github.com/uncleoptimus/udacityP4/blob/gh-pages/views/js/main.js
    var cols = Math.ceil(window.innerWidth / (s - 73.33));
